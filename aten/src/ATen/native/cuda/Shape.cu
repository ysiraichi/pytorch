...
void hip_parallel_cat(const Tensor &out, ITensorListRef inputs, int64_t dimension,
  ...
  int batchCounter = 0;
  int64_t offset = 0;
  for (int i = 0; i < materialized.size() ; i += CAT_ARRAY_BATCH_SIZE) {
    // Re-allocate stackInputs every iteration to avoid read-after-write hazard
    {
      ...
      for (batchCounter = 0;
           batchCounter < CAT_ARRAY_BATCH_SIZE &&
             (i+batchCounter) < materialized.size();
           ++batchCounter) {
        int64_t dimSize = 0;
        // There is a legacy case where a 1-D empty tensor can be concat with
        // high-dimensional tensor
        if (materialized[i+batchCounter].get().numel() > 0) {
          dimSize = at::native::size(materialized[i+batchCounter], dimension);
        }

        stackInputs[batchCounter].input =
          materialized[i+batchCounter].get().data_ptr<scalar_t>();
        stackInputs[batchCounter].offset = offset;
        stackInputs[batchCounter].dimSize = dimSize;
        stackInputs[batchCounter].nElements = materialized[i+batchCounter].get().numel();
...
void parallel_cat(const Tensor &out, ITensorListRef inputs, int64_t dimension,
  ...
  int batchCounter = 0;
  int64_t offset = 0;
  for (int i = 0; i < materialized.size() ; i += batch_size) {
    for (batchCounter = 0;
      ...
      catMetaData.input[batchCounter] = materialized[i+batchCounter].get().data_ptr<scalar_t>();
      catMetaData.offset[batchCounter] = offset;
      catMetaData.dimSize[batchCounter] = dimSize;
      catMetaData.nElements[batchCounter] = materialized[i+batchCounter].get().numel();
      if (stride_size > 1) {
        auto strides = materialized[i+batchCounter].get().strides();
        auto sizes = materialized[i+batchCounter].get().sizes();
...
(ITensorListRef tensors,
 ...
 const Tensor& result) {
  ...
  if (materialized.size() > 1 &&
    ...
    int64_t offset = 0;
    for (int j = 0; j < materialized.size(); j++) {
      if (cat_should_skip_tensor(materialized[j])) continue;
      int64_t dimSize = at::native::size(materialized[j], dim);
      Tensor nt = at::narrow(result, dim, offset, dimSize);
      copy_(nt, materialized[j]);
      offset += dimSize;
    }
  }
}
